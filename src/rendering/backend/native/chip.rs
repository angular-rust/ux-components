...
#[derive(Debug)]
pub struct Chip {
    props: RefCell<ChipProps>,
    inner: Widget,
}

impl Chip {
    pub fn new() -> Self {
        let props = ChipProps::default();
...
impl Is<Widget> for Chip {}

impl AsRef<Widget> for Chip {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for Chip {}

impl AsRef<Actor> for Chip {
...
pub trait ChipExt: 'static {
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @Chip will take its label and
    /// icon from @action.
    ///

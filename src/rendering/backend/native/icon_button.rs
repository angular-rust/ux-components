...
pub struct IconButton {
    props: RefCell<IconButtonProps>,
    inner: Widget,
}

impl IconButton {
    pub fn new() -> Self {
        let props = IconButtonProps::default();
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = IconButtonProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO ICON BUTTON [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO ICON BUTTON [{}]", text);
...
impl Is<IconButton> for IconButton {}

impl AsRef<IconButton> for IconButton {
    fn as_ref(&self) -> &IconButton {
        self
    }
}

impl Is<Widget> for IconButton {}

impl AsRef<Widget> for IconButton {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for IconButton {}

impl AsRef<Actor> for IconButton {
...
pub trait IconButtonExt: 'static {
    /// get_action:
    /// @button: A #IconButton
    ///
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @IconButton will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<IconButton>> IconButtonExt for O {
    /// get_action:
    /// @button: A #IconButton
    ///
    /// Retrieves the #PushAction associated with @button.
    ...
    fn connect_clicked<F: Fn(&Self) + 'static>(&self, f: F) -> HandlerId {
        ...
        //     this: *mut ffi::IconButton,
        //     f: glib_sys::gpointer,
        // ) where
        //     P: Is<IconButton>,
        // {
        //     let f: &F = &*(f as *const F);
        //     f(&IconButton::from_glib_borrow(this).unsafe_cast_ref())
        // }
        // unsafe {

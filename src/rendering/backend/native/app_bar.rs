...
#[derive(Debug)]
pub struct AppBar {
    props: RefCell<AppBarProps>,
    inner: Widget,
}

impl AppBar {
    pub fn new() -> Self {
        let props = AppBarProps::default();
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = AppBarProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO APPBAR [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO APPBAR [{}]", text);
...
impl Object for AppBar {}
impl Is<AppBar> for AppBar {}

impl AsRef<AppBar> for AppBar {
    fn as_ref(&self) -> &AppBar {
        self
    }
}

impl Is<Widget> for AppBar {}

impl AsRef<Widget> for AppBar {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for AppBar {}

impl AsRef<Actor> for AppBar {
...
pub trait AppBarExt: 'static {
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @AppBar will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<AppBar>> AppBarExt for O {
    /// get_action:
    /// @button: A #AppBar
    ///
    /// Retrieves the #PushAction associated with @button.

...
#[derive(Debug)]
pub struct Snackbar {
    props: RefCell<SnackbarProps>,
    inner: Widget,
}

impl Snackbar {
    pub fn new() -> Self {
        let props = SnackbarProps::default();
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = SnackbarProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO SNACKBAR [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO SNACKBAR [{}]", text);
...
impl Object for Snackbar {}
impl Is<Snackbar> for Snackbar {}

impl AsRef<Snackbar> for Snackbar {
    fn as_ref(&self) -> &Snackbar {
        self
    }
}

impl Is<Widget> for Snackbar {}

impl AsRef<Widget> for Snackbar {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for Snackbar {}

impl AsRef<Actor> for Snackbar {
...
pub trait SnackbarExt: 'static {
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @Snackbar will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<Snackbar>> SnackbarExt for O {
    /// get_action:
    /// @button: A #Snackbar
    ///
    /// Retrieves the #PushAction associated with @button.
    ...
    fn connect_clicked<F: Fn(&Self) + 'static>(&self, f: F) -> HandlerId {
        ...
        //     this: *mut ffi::Snackbar,
        //     f: glib_sys::gpointer,
        // ) where
        //     P: Is<Snackbar>,
        // {
        //     let f: &F = &*(f as *const F);
        //     f(&Snackbar::from_glib_borrow(this).unsafe_cast_ref())
        // }
        // unsafe {

...
#[derive(Debug)]
pub struct Tab {
    props: RefCell<TabProps>,
    inner: Widget,
}

impl Tab {
    pub fn new() -> Self {
        let props = TabProps::default();
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = TabProps::default();
        if !text.is_empty() {
...
impl AsRef<Tab> for Tab {
    fn as_ref(&self) -> &Tab {
        self
    }
}

impl Is<Widget> for Tab {}

impl AsRef<Widget> for Tab {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for Tab {}

impl AsRef<Actor> for Tab {
...
pub trait TabExt: 'static {
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @Tab will take its label and
    /// icon from @action.
    ///

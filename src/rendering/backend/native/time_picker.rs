...
pub struct TimePicker {
    props: RefCell<TimePickerProps>,
    inner: Widget,
}

impl TimePicker {
    pub fn new() -> Self {
        let props = TimePickerProps::default();
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = TimePickerProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO TIME PICKER [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO TIME PICKER [{}]", text);
...
impl Object for TimePicker {}
impl Is<TimePicker> for TimePicker {}

impl AsRef<TimePicker> for TimePicker {
    fn as_ref(&self) -> &TimePicker {
        self
    }
}

impl Is<Widget> for TimePicker {}

impl AsRef<Widget> for TimePicker {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for TimePicker {}

impl AsRef<Actor> for TimePicker {
...
pub trait TimePickerExt: 'static {
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @TimePicker will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<TimePicker>> TimePickerExt for O {
    /// get_action:
    /// @button: A #TimePicker
    ///
    /// Retrieves the #PushAction associated with @button.

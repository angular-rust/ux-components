...
impl DatePicker {
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = DatePickerProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO DATE PICKER [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO DATE PICKER [{}]", text);
...
impl Is<DatePicker> for DatePicker {}

impl AsRef<DatePicker> for DatePicker {
    fn as_ref(&self) -> &DatePicker {
        self
    }
}

impl Is<Widget> for DatePicker {}

impl AsRef<Widget> for DatePicker {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for DatePicker {}

impl AsRef<Actor> for DatePicker {
...
pub trait DatePickerExt: 'static {
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @DatePicker will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<DatePicker>> DatePickerExt for O {
    /// get_action:
    /// @button: A #DatePicker
    ///
    /// Retrieves the #PushAction associated with @button.

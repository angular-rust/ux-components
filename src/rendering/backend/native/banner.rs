...
#[derive(Debug)]
pub struct Banner {
    props: RefCell<BannerProps>,
    inner: Widget,
}

impl Banner {
    pub fn new() -> Self {
        let props = BannerProps::default();
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = BannerProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO BANNER [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO BANNER [{}]", text);
...
impl Object for Banner {}
impl Is<Banner> for Banner {}

impl AsRef<Banner> for Banner {
    fn as_ref(&self) -> &Banner {
        self
    }
}

impl Is<Widget> for Banner {}

impl AsRef<Widget> for Banner {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for Banner {}

impl AsRef<Actor> for Banner {
...
pub trait BannerExt: 'static {
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @Banner will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<Banner>> BannerExt for O {
    /// get_action:
    /// @button: A #Banner
    ///
    /// Retrieves the #PushAction associated with @button.

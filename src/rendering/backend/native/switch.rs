...
impl Switch {
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = SwitchProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO SWITCH [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO SWITCH [{}]", text);
...
impl Is<Widget> for Switch {}

impl AsRef<Widget> for Switch {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for Switch {}

impl AsRef<Actor> for Switch {
...
pub trait SwitchExt: 'static {
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @Switch will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<Switch>> SwitchExt for O {
    /// get_action:
    /// @button: A #Switch
    ///
    /// Retrieves the #PushAction associated with @button.

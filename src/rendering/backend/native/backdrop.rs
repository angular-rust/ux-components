...
use crate::{
    foundation::platform::core::Texture, prelude::*, Actor, HandlerId, Icon, Position, PushAction,
    StyleClass, Text, Theme, Widget,
};
...
#[derive(Debug)]
pub struct Backdrop {
    props: RefCell<BackdropProps>,
    inner: Widget,
}

impl Backdrop {
    pub fn new() -> Self {
        let props = BackdropProps::default();
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = BackdropProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO BACKDROP [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO BACKDROP [{}]", text);
...
impl Is<Backdrop> for Backdrop {}

impl AsRef<Backdrop> for Backdrop {
    fn as_ref(&self) -> &Backdrop {
        self
    }
}

impl Is<Widget> for Backdrop {}

impl AsRef<Widget> for Backdrop {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for Backdrop {}

impl AsRef<Actor> for Backdrop {
...
pub trait BackdropExt: 'static {
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @Backdrop will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<Backdrop>> BackdropExt for O {
    /// get_action:
    /// @button: A #Backdrop
    ///
    /// Retrieves the #PushAction associated with @button.
    ...
    fn connect_clicked<F: Fn(&Self) + 'static>(&self, f: F) -> HandlerId {
        ...
        //     this: *mut ffi::Backdrop,
        //     f: glib_sys::gpointer,
        // ) where
        //     P: Is<Backdrop>,
        // {
        //     let f: &F = &*(f as *const F);
        //     f(&Backdrop::from_glib_borrow(this).unsafe_cast_ref())
        // }
        // unsafe {

...
impl Alert {
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = AlertProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO ALERT [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO ALERT [{}]", text);
...
impl Object for Alert {}
impl Is<Alert> for Alert {}

impl AsRef<Alert> for Alert {
    fn as_ref(&self) -> &Alert {
        self
    }
}

impl Is<Widget> for Alert {}

impl AsRef<Widget> for Alert {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for Alert {}

impl AsRef<Actor> for Alert {
...
pub trait AlertExt: 'static {
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @Alert will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<Alert>> AlertExt for O {
    /// get_action:
    /// @button: A #Alert
    ///
    /// Retrieves the #PushAction associated with @button.
    ...
    fn connect_clicked<F: Fn(&Self) + 'static>(&self, f: F) -> HandlerId {
        ...
        //     f(&Alert::from_glib_borrow(this).unsafe_cast_ref())
        // }
        // unsafe {

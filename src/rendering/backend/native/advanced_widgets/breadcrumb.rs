...
impl Breadcrumb {
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = BreadcrumbProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO BRADCRUMB [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO BRADCRUMB [{}]", text);
...
impl Is<Breadcrumb> for Breadcrumb {}

impl AsRef<Breadcrumb> for Breadcrumb {
    fn as_ref(&self) -> &Breadcrumb {
        self
    }
}

impl Is<Widget> for Breadcrumb {}

impl AsRef<Widget> for Breadcrumb {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for Breadcrumb {}

impl AsRef<Actor> for Breadcrumb {
...
pub trait BreadcrumbExt: 'static {
    /// get_action:
    /// @button: A #Breadcrumb
    ///
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @Breadcrumb will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<Breadcrumb>> BreadcrumbExt for O {
    /// get_action:
    /// @button: A #Breadcrumb
    ///
    /// Retrieves the #PushAction associated with @button.
    ...
    fn connect_clicked<F: Fn(&Self) + 'static>(&self, f: F) -> HandlerId {
        ...
        //     this: *mut ffi::Breadcrumb,
        //     f: glib_sys::gpointer,
        // ) where
        //     P: Is<Breadcrumb>,
        // {
        //     let f: &F = &*(f as *const F);
        //     f(&Breadcrumb::from_glib_borrow(this).unsafe_cast_ref())
        // }
        // unsafe {

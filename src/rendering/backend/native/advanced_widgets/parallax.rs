...
#[derive(Debug)]
pub struct Parallax {
    props: RefCell<ParallaxProps>,
    inner: Widget,
}

impl Parallax {
    pub fn new() -> Self {
        let props = ParallaxProps::default();
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = ParallaxProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO PARALAX [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO PARALAX [{}]", text);
...
impl Object for Parallax {}
impl Is<Parallax> for Parallax {}

impl AsRef<Parallax> for Parallax {
    fn as_ref(&self) -> &Parallax {
        self
    }
}

impl Is<Widget> for Parallax {}

impl AsRef<Widget> for Parallax {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for Parallax {}

impl AsRef<Actor> for Parallax {
...
pub trait ParallaxExt: 'static {
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @Parallax will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<Parallax>> ParallaxExt for O {
    /// get_action:
    /// @button: A #Parallax
    ///
    /// Retrieves the #PushAction associated with @button.
    ...
    fn connect_clicked<F: Fn(&Self) + 'static>(&self, f: F) -> HandlerId {
        ...
        //     this: *mut ffi::Parallax,
        //     f: glib_sys::gpointer,
        // ) where
        //     P: Is<Parallax>,
        // {
        //     let f: &F = &*(f as *const F);
        //     f(&Parallax::from_glib_borrow(this).unsafe_cast_ref())
        // }
        // unsafe {

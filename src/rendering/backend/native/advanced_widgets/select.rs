...
impl Select {
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = SelectProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO SELECT [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO SELECT [{}]", text);
...
impl Is<Select> for Select {}

impl AsRef<Select> for Select {
    fn as_ref(&self) -> &Select {
        self
    }
}

impl Is<Widget> for Select {}

impl AsRef<Widget> for Select {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for Select {}

impl AsRef<Actor> for Select {
...
pub trait SelectExt: 'static {
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @Select will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<Select>> SelectExt for O {
    /// get_action:
    /// @button: A #Select
    ///
    /// Retrieves the #PushAction associated with @button.
    ...
    fn connect_clicked<F: Fn(&Self) + 'static>(&self, f: F) -> HandlerId {
        ...
        //     f(&Select::from_glib_borrow(this).unsafe_cast_ref())
        // }
        // unsafe {

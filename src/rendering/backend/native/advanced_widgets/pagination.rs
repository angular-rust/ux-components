...
impl Pagination {
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = PaginationProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO PAGINATION [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO PAGINATION [{}]", text);
...
impl Is<Pagination> for Pagination {}

impl AsRef<Pagination> for Pagination {
    fn as_ref(&self) -> &Pagination {
        self
    }
}

impl Is<Widget> for Pagination {}

impl AsRef<Widget> for Pagination {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for Pagination {}

impl AsRef<Actor> for Pagination {
...
pub trait PaginationExt: 'static {
    /// get_action:
    /// @button: A #Pagination
    ///
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @Pagination will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<Pagination>> PaginationExt for O {
    /// get_action:
    /// @button: A #Pagination
    ///
    /// Retrieves the #PushAction associated with @button.
    ...
    fn connect_clicked<F: Fn(&Self) + 'static>(&self, f: F) -> HandlerId {
        ...
        //     this: *mut ffi::Pagination,
        //     f: glib_sys::gpointer,
        // ) where
        //     P: Is<Pagination>,
        // {
        //     let f: &F = &*(f as *const F);
        //     f(&Pagination::from_glib_borrow(this).unsafe_cast_ref())
        // }
        // unsafe {

...
pub struct ColorPicker {
    props: RefCell<ColorPickerProps>,
    inner: Widget,
}
...
impl ColorPicker {
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = ColorPickerProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO COLOR PICKER [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO COLOR PICKER [{}]", text);
...
impl Is<ColorPicker> for ColorPicker {}

impl AsRef<ColorPicker> for ColorPicker {
    fn as_ref(&self) -> &ColorPicker {
        self
    }
}

impl Is<Widget> for ColorPicker {}

impl AsRef<Widget> for ColorPicker {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for ColorPicker {}

impl AsRef<Actor> for ColorPicker {
...
pub trait ColorPickerExt: 'static {
    /// get_action:
    /// @button: A #ColorPicker
    ///
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @ColorPicker will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<ColorPicker>> ColorPickerExt for O {
    /// get_action:
    /// @button: A #ColorPicker
    ///
    /// Retrieves the #PushAction associated with @button.
    ...
    fn connect_clicked<F: Fn(&Self) + 'static>(&self, f: F) -> HandlerId {
        ...
        //     this: *mut ffi::ColorPicker,
        //     f: glib_sys::gpointer,
        // ) where
        //     P: Is<ColorPicker>,
        // {
        //     let f: &F = &*(f as *const F);
        //     f(&ColorPicker::from_glib_borrow(this).unsafe_cast_ref())
        // }
        // unsafe {
...
impl fmt::Display for ColorPicker {

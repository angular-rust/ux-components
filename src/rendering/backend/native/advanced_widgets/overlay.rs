...
impl Overlay {
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = OverlayProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO OVERLAY [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO OVERLAY [{}]", text);
...
impl Is<Overlay> for Overlay {}

impl AsRef<Overlay> for Overlay {
    fn as_ref(&self) -> &Overlay {
        self
    }
}

impl Is<Widget> for Overlay {}

impl AsRef<Widget> for Overlay {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for Overlay {}

impl AsRef<Actor> for Overlay {
...
pub trait OverlayExt: 'static {
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @Overlay will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<Overlay>> OverlayExt for O {
    /// get_action:
    /// @button: A #Overlay
    ///
    /// Retrieves the #PushAction associated with @button.

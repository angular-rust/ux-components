...
impl Stepper {
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = StepperProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO STEPPER [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO STEPPER [{}]", text);
...
impl Object for Stepper {}
impl Is<Stepper> for Stepper {}

impl AsRef<Stepper> for Stepper {
    fn as_ref(&self) -> &Stepper {
        self
    }
}

impl Is<Widget> for Stepper {}

impl AsRef<Widget> for Stepper {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for Stepper {}

impl AsRef<Actor> for Stepper {
...
pub trait StepperExt: 'static {
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @Stepper will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<Stepper>> StepperExt for O {
    /// get_action:
    /// @button: A #Stepper
    ///
    /// Retrieves the #PushAction associated with @button.
    ...
    fn connect_clicked<F: Fn(&Self) + 'static>(&self, f: F) -> HandlerId {
        ...
        //     this: *mut ffi::Stepper,
        //     f: glib_sys::gpointer,
        // ) where
        //     P: Is<Stepper>,
        // {
        //     let f: &F = &*(f as *const F);
        //     f(&Stepper::from_glib_borrow(this).unsafe_cast_ref())
        // }
        // unsafe {

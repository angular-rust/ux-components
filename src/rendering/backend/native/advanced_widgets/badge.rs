...
impl Badge {
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = BadgeProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO BADGE [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO BADGE [{}]", text);
...
impl Is<Badge> for Badge {}

impl AsRef<Badge> for Badge {
    fn as_ref(&self) -> &Badge {
        self
    }
}

impl Is<Widget> for Badge {}

impl AsRef<Widget> for Badge {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for Badge {}

impl AsRef<Actor> for Badge {
...
pub trait BadgeExt: 'static {
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @Badge will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<Badge>> BadgeExt for O {
    /// get_action:
    /// @button: A #Badge
    ///
    /// Retrieves the #PushAction associated with @button.

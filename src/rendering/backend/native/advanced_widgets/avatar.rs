...
#[derive(Debug)]
pub struct Avatar {
    props: RefCell<AvatarProps>,
    inner: Widget,
}

impl Avatar {
    pub fn new() -> Self {
        let props = AvatarProps::default();
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = AvatarProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO AVATAR [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO AVATAR [{}]", text);
...
impl Is<Avatar> for Avatar {}

impl AsRef<Avatar> for Avatar {
    fn as_ref(&self) -> &Avatar {
        self
    }
}

impl Is<Widget> for Avatar {}

impl AsRef<Widget> for Avatar {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for Avatar {}

impl AsRef<Actor> for Avatar {
...
pub trait AvatarExt: 'static {
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @Avatar will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<Avatar>> AvatarExt for O {
    /// get_action:
    /// @button: A #Avatar
    ///
    /// Retrieves the #PushAction associated with @button.

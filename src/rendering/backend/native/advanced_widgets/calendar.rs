...
impl Calendar {
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = CalendarProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO CALENDAR [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO CALENDAR [{}]", text);
...
impl Object for Calendar {}
impl Is<Calendar> for Calendar {}

impl AsRef<Calendar> for Calendar {
    fn as_ref(&self) -> &Calendar {
        self
    }
}

impl Is<Widget> for Calendar {}

impl AsRef<Widget> for Calendar {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for Calendar {}

impl AsRef<Actor> for Calendar {
...
pub trait CalendarExt: 'static {
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @Calendar will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<Calendar>> CalendarExt for O {
    /// get_action:
    /// @button: A #Calendar
    ///
    /// Retrieves the #PushAction associated with @button.

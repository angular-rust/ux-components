...
use crate::{platform::core::Texture, Actor, HandlerId, Position, PushAction, Text, Widget};
...
pub struct CircularProgressProps {
    ...
    pub content_image: Option<Texture>,
...
pub struct CircularProgress {
    props: RefCell<CircularProgressProps>,
    inner: Widget,
}

impl CircularProgress {
    pub fn new() -> Self {
        let props = CircularProgressProps::default();
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = CircularProgressProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        println!("INIT CIRCULAR PROGRESS");
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO CIRCULAR PROGRESS [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO CIRCULAR PROGRESS [{}]", text);
...
impl Object for CircularProgress {}
impl Is<CircularProgress> for CircularProgress {}

impl AsRef<CircularProgress> for CircularProgress {
    fn as_ref(&self) -> &CircularProgress {
        self
    }
}

impl Is<Widget> for CircularProgress {}

impl AsRef<Widget> for CircularProgress {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for CircularProgress {}

impl AsRef<Actor> for CircularProgress {
...
pub trait CircularProgressExt: 'static {
    /// get_action:
    /// @button: A #CircularProgress
    ///
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @CircularProgress will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<CircularProgress>> CircularProgressExt for O {
    /// get_action:
    /// @button: A #CircularProgress
    ///
    /// Retrieves the #PushAction associated with @button.
    ...
    fn connect_clicked<F: Fn(&Self) + 'static>(&self, f: F) -> HandlerId {
        ...
        //     this: *mut ffi::CircularProgress,
        //     f: glib_sys::gpointer,
        // ) where
        //     P: Is<CircularProgress>,
        // {
...
impl fmt::Display for CircularProgress {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "CircularProgress")
    }
}

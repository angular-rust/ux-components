...
impl Divider {
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = DividerProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO DIVIDER [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO DIVIDER [{}]", text);
...
impl Object for Divider {}
impl Is<Divider> for Divider {}

impl AsRef<Divider> for Divider {
    fn as_ref(&self) -> &Divider {
        self
    }
}

impl Is<Widget> for Divider {}

impl AsRef<Widget> for Divider {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for Divider {}

impl AsRef<Actor> for Divider {
...
pub trait DividerExt: 'static {
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @Divider will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<Divider>> DividerExt for O {
    /// get_action:
    /// @button: A #Divider
    ///
    /// Retrieves the #PushAction associated with @button.

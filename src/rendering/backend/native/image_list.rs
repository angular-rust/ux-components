...
pub struct ImageList {
    props: RefCell<ImageListProps>,
    inner: Widget,
}

impl ImageList {
    pub fn new() -> Self {
        let props = ImageListProps::default();
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = ImageListProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO IMAGE LIST [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO IMAGE LIST [{}]", text);
...
impl Object for ImageList {}
impl Is<ImageList> for ImageList {}

impl AsRef<ImageList> for ImageList {
    fn as_ref(&self) -> &ImageList {
        self
    }
}

impl Is<Widget> for ImageList {}

impl AsRef<Widget> for ImageList {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for ImageList {}

impl AsRef<Actor> for ImageList {
...
pub trait ImageListExt: 'static {
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @ImageList will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<ImageList>> ImageListExt for O {
    /// get_action:
    /// @button: A #ImageList
    ///
    /// Retrieves the #PushAction associated with @button.
    ...
    fn connect_clicked<F: Fn(&Self) + 'static>(&self, f: F) -> HandlerId {
        ...
        //     this: *mut ffi::ImageList,
        //     f: glib_sys::gpointer,
        // ) where
        //     P: Is<ImageList>,
        // {
        //     let f: &F = &*(f as *const F);
        //     f(&ImageList::from_glib_borrow(this).unsafe_cast_ref())
        // }
        // unsafe {

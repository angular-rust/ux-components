...
impl Radio {
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = RadioProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO RADIO [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO RADIO [{}]", text);
...
impl Object for Radio {}
impl Is<Radio> for Radio {}

impl AsRef<Radio> for Radio {
    fn as_ref(&self) -> &Radio {
        self
    }
}

impl Is<Widget> for Radio {}

impl AsRef<Widget> for Radio {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for Radio {}

impl AsRef<Actor> for Radio {
...
pub trait RadioExt: 'static {
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @Radio will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<Radio>> RadioExt for O {
    /// get_action:
    /// @button: A #Radio
    ///
    /// Retrieves the #PushAction associated with @button.
    ...
    fn connect_clicked<F: Fn(&Self) + 'static>(&self, f: F) -> HandlerId {
        ...
        //     f(&Radio::from_glib_borrow(this).unsafe_cast_ref())
        // }
        // unsafe {

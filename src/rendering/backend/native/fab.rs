...
#[derive(Debug)]
pub struct Fab {
    props: RefCell<FabProps>,
    inner: Widget,
}

impl Fab {
    pub fn new() -> Self {
        let props = FabProps::default();
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = FabProps::default();
        if !text.is_empty() {
...
impl AsRef<Fab> for Fab {
    fn as_ref(&self) -> &Fab {
        self
    }
}

impl Is<Widget> for Fab {}

impl AsRef<Widget> for Fab {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for Fab {}

impl AsRef<Actor> for Fab {
...
pub trait FabExt: 'static {
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @Fab will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<Fab>> FabExt for O {
    ...
    fn connect_clicked<F: Fn(&Self) + 'static>(&self, f: F) -> HandlerId {
        ...
        //     f(&Fab::from_glib_borrow(this).unsafe_cast_ref())
        // }
        // unsafe {

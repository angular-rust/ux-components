...
#[derive(Debug)]
pub struct Sheet {
    props: RefCell<SheetProps>,
    inner: Widget,
}

impl Sheet {
    pub fn new() -> Self {
        let props = SheetProps::default();
    ...
    pub fn with_label(text: &str) -> Self {
        let mut props = SheetProps::default();
        if !text.is_empty() {
    ...
    fn init(&self) {
        ...
        if let Some(name) = &props.icon_name {
            println!("ADD ICON TO SHEET [{}]", name);
            let icon = Icon::new();
            // priv->icon = icon_new ();
        ...
        if let Some(text) = &props.text {
            println!("ADD TEXT TO SHEET [{}]", text);
...
impl Object for Sheet {}
impl Is<Sheet> for Sheet {}

impl AsRef<Sheet> for Sheet {
    fn as_ref(&self) -> &Sheet {
        self
    }
}

impl Is<Widget> for Sheet {}

impl AsRef<Widget> for Sheet {
    fn as_ref(&self) -> &Widget {
        &self.inner
    }
}

impl Is<Actor> for Sheet {}

impl AsRef<Actor> for Sheet {
...
pub trait SheetExt: 'static {
    ...
    /// @action: A #PushAction
    ///
    /// Sets @action as the action for @button. @Sheet will take its label and
    /// icon from @action.
    ///
...
impl<O: Is<Sheet>> SheetExt for O {
    /// get_action:
    /// @button: A #Sheet
    ///
    /// Retrieves the #PushAction associated with @button.
    ...
    fn connect_clicked<F: Fn(&Self) + 'static>(&self, f: F) -> HandlerId {
        ...
        //     f(&Sheet::from_glib_borrow(this).unsafe_cast_ref())
        // }
        // unsafe {

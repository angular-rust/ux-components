...
impl Debug for NavigationToolbarElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("NavigationToolbarElement").finish()
    }
}

impl NavigationToolbarElement {
    pub fn new(widget: &NavigationToolbar) -> Self {
...
impl AsRef<RefCell<WidgetComponent>> for NavigationToolbarElement {
...
impl Element for NavigationToolbarElement {
    ...
    fn mousedown(&self, e: &mut MouseEvent) {
        ...
        if inside {
            ...
            if self.middle.contains(x, y) {
                log::info!("Is an Title");
                self.middle.mousedown(e);
            }

            if self.trailing.contains(x, y) {
                // log::info!("Is an FAB");
    ...
    fn update(&self, dt: f32) {
        log::info!("Update NavigationToolbar");
        let component = self.component.borrow();

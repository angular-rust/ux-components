...
struct OffstageState {
    depth_idx: f32, // = 0.0;

    mouse_down: bool, // = false;
}

/// A Offstage is a layout-only object and it do not requires a rendering instance.
/// It and handles all incoming events, propagating them to the children.
/// Additional Signals: none

pub struct OffstageElement {
    ...
    state: RefCell<OffstageState>,

    // The node in layout system
    pub node: Node,
}

impl Debug for OffstageElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("OffstageElement").finish()
    }
}

impl OffstageElement {
    pub fn new(widget: &Offstage) -> Self {
        ...
        //     log::warn!("Got signal to render Offstage");
        // });
...
impl AsRef<RefCell<WidgetComponent>> for OffstageElement {
...
impl Element for OffstageElement {
    ...
    fn update(&self, dt: f32) {
        log::info!("Update OffstageElement");
        let component = self.component.borrow();

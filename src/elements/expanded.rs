...
/// A Expanded is a layout-only object and it do not requires a rendering instance.
...
pub struct ExpandedElement {
    ...
    state: RefCell<ExpandedState>,
    // The node in layout system
    pub node: Node,
}

impl Debug for ExpandedElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("ExpandedElement").finish()
    }
}
...
impl ExpandedElement {
    pub fn new(widget: &Expanded) -> Self {
        ...
        //     log::warn!("Got signal to render Expanded");
        // });
...
impl AsRef<RefCell<WidgetComponent>> for ExpandedElement {
...
impl Element for ExpandedElement {

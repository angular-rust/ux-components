...
/// A AlertDialog is a layout-only object and it do not requires a rendering instance.
/// It and handles all incoming events, propagating them to the children.
/// Additional Signals: none

pub struct AlertDialogElement {
    ...
    state: RefCell<AlertDialogState>,
    // The node in layout system
    pub node: Node,
}

impl Debug for AlertDialogElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("AlertDialogElement").finish()
    }
}

impl AlertDialogElement {
    pub fn new(widget: &AlertDialog) -> Self {
        ...
        //     log::warn!("Got signal to render AlertDialog");
        // });
...
impl AsRef<RefCell<WidgetComponent>> for AlertDialogElement {
...
impl Element for AlertDialogElement {
    ...
    fn mousedown(&self, e: &mut MouseEvent) {
        ...
        if inside {
            ...
            if self.child.contains(x, y) {
                log::info!("Is an AlertDialog Child");

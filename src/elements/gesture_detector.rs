...
/// A GestureDetector is a layout-only object and it do not requires a rendering instance.
...
pub struct GestureDetectorElement {
    ...
    state: RefCell<GestureDetectorState>,

    // The node in layout system
    pub node: Node,
}

impl Debug for GestureDetectorElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("GestureDetectorElement").finish()
    }
}

impl GestureDetectorElement {
    pub fn new(widget: &GestureDetector) -> Self {
        ...
        //     log::warn!("Got signal to render GestureDetector");
        // });
...
impl AsRef<RefCell<WidgetComponent>> for GestureDetectorElement {
...
impl Element for GestureDetectorElement {
    ...
    fn update(&self, dt: f32) {
        log::info!("Update GestureDetectorElement");
        let component = self.component.borrow();

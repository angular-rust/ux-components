...
/// A ListView is a layout-only object and it do not requires a rendering instance.
...
pub struct ListViewElement {
    ...
    state: RefCell<ListViewState>,
    // The node in layout system
    pub node: Node,
}

impl Debug for ListViewElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("ListViewElement").finish()
    }
}

impl ListViewElement {
    pub fn new(widget: &ListView) -> Self {
        ...
        //     log::warn!("Got signal to render ListView");
        // });
...
impl AsRef<RefCell<WidgetComponent>> for ListViewElement {
...
impl Element for ListViewElement {
    ...
    fn update(&self, dt: f32) {
        log::info!("Update ListViewElement");
        // let mut component = self.component.borrow_mut();

...
/// A ClipPath is a layout-only object and it do not requires a rendering instance.
...
pub struct ClipPathElement {
    ...
    state: RefCell<ClipPathState>,

    // The node in layout system
    pub node: Node,
}

impl Debug for ClipPathElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("ClipPathElement").finish()
    }
}

impl ClipPathElement {
    pub fn new(widget: &ClipPath) -> Self {
        ...
        //     log::warn!("Got signal to render ClipPath");
        // });
...
impl AsRef<RefCell<WidgetComponent>> for ClipPathElement {
...
impl Element for ClipPathElement {
    ...
    fn update(&self, dt: f32) {
        log::info!("Update ClipPathElement");
        let component = self.component.borrow();

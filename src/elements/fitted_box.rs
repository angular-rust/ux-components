...
struct FittedBoxState {
    depth_idx: f32, // = 0.0;

    mouse_down: bool, // = false;
}

/// A FittedBox is a layout-only object and it do not requires a rendering instance.
/// It and handles all incoming events, propagating them to the children.
/// Additional Signals: none

pub struct FittedBoxElement {
    ...
    state: RefCell<FittedBoxState>,

    // The node in layout system
    pub node: Node,
}

impl Debug for FittedBoxElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("FittedBoxElement").finish()
    }
}

impl FittedBoxElement {
    pub fn new(widget: &FittedBox) -> Self {
        ...
        //     log::warn!("Got signal to render FittedBox");
        // });
...
impl AsRef<RefCell<WidgetComponent>> for FittedBoxElement {
...
impl Element for FittedBoxElement {
    ...
    fn update(&self, dt: f32) {
        log::info!("Update FittedBoxElement");
        let component = self.component.borrow();

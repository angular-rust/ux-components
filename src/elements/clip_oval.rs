...
struct ClipOvalState {
    depth_idx: f32, // = 0.0;

    mouse_down: bool, // = false;
}

/// A ClipOval is a layout-only object and it do not requires a rendering instance.
/// It and handles all incoming events, propagating them to the children.
/// Additional Signals: none

pub struct ClipOvalElement {
    ...
    state: RefCell<ClipOvalState>,

    // The node in layout system
    pub node: Node,
}

impl Debug for ClipOvalElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("ClipOvalElement").finish()
    }
}

impl ClipOvalElement {
    pub fn new(widget: &ClipOval) -> Self {
        ...
        //     log::warn!("Got signal to render ClipOval");
        // });
...
impl AsRef<RefCell<WidgetComponent>> for ClipOvalElement {
...
impl Element for ClipOvalElement {
    ...
    fn update(&self, dt: f32) {
        log::info!("Update ClipOvalElement");
        let component = self.component.borrow();

...
/// A Flexible is a layout-only object and it do not requires a rendering instance.
...
pub struct FlexibleElement {
    ...
    state: RefCell<FlexibleState>,

    // The node in layout system
    pub node: Node,
}

impl Debug for FlexibleElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("FlexibleElement").finish()
    }
}

impl FlexibleElement {
    pub fn new(widget: &Flexible) -> Self {
        ...
        //     log::warn!("Got signal to render Flexible");
        // });
...
impl AsRef<RefCell<WidgetComponent>> for FlexibleElement {
...
impl Element for FlexibleElement {
    ...
    fn update(&self, dt: f32) {
        log::info!("Update FlexibleElement");
        let component = self.component.borrow();

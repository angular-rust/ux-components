...
/// A Material is a layout-only object and it do not requires a rendering instance.
...
pub struct MaterialElement {
    ...
    state: RefCell<MaterialState>,

    // The node in layout system
    pub node: Node,
}

impl Debug for MaterialElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("MaterialElement").finish()
    }
}

impl MaterialElement {
    pub fn new(widget: &Material) -> Self {
        ...
        //     log::warn!("Got signal to render Material");
        // });
...
impl AsRef<RefCell<WidgetComponent>> for MaterialElement {
...
impl Element for MaterialElement {
    ...
    fn update(&self, dt: f32) {
        log::info!("Update MaterialElement");
        let component = self.component.borrow();

...
pub struct PopupMenuButtonElement {
...
impl Debug for PopupMenuButtonElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("ButtonElement").finish()
    }
}

impl PopupMenuButtonElement {
    pub fn new(widget: &PopupMenuButton) -> Self {
...
impl AsRef<RefCell<WidgetComponent>> for PopupMenuButtonElement {
    fn as_ref(&self) -> &RefCell<WidgetComponent> {
        self.component.as_ref()
    }
}

impl Element for PopupMenuButtonElement {
    fn mousedown(&self, e: &mut MouseEvent) {
        ...
        log::info!("PopupMenuButtonElement clicked");
        // self.state.borrow_mut().mouse_down = true;
        // let component = self.component.borrow();
        // component.onmousedown.emit(e);
    ...
    fn render(&self) {
        ...
        log::warn!("Render PopupMenuButtonElement");
    ...
    fn relayout(&self, origin: Point2<f32>) {
        let update_childs = match LayoutSystem::layout(self.node) {
            Ok(layout) => {
                ...
                log::warn!(
                    "Relayout PopupMenuButtonElement {}x{} {}x{}",
                    comp.x,
                    comp.y,
                    comp.w,
                    comp.h

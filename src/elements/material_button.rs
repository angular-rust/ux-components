...
pub struct MaterialButtonElement {
...
impl Debug for MaterialButtonElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("ButtonElement").finish()
    }
}

impl MaterialButtonElement {
    pub fn new(widget: &MaterialButton) -> Self {
...
impl AsRef<RefCell<WidgetComponent>> for MaterialButtonElement {
    fn as_ref(&self) -> &RefCell<WidgetComponent> {
        self.component.as_ref()
    }
}

impl Element for MaterialButtonElement {
    fn mousedown(&self, e: &mut MouseEvent) {
        ...
        log::info!("MaterialButtonElement clicked");
        // self.state.borrow_mut().mouse_down = true;
        // let component = self.component.borrow();
        // component.onmousedown.emit(e);

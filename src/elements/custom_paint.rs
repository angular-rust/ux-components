...
struct CustomPaintState {
    depth_idx: f32, // = 0.0;

    mouse_down: bool, // = false;
}

/// A CustomPaint is a layout-only object and it do not requires a rendering instance.
/// It and handles all incoming events, propagating them to the children.
/// Additional Signals: none

pub struct CustomPaintElement {
    ...
    state: RefCell<CustomPaintState>,

    // The node in layout system
    pub node: Node,
}

impl Debug for CustomPaintElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("CustomPaintElement").finish()
    }
}

impl CustomPaintElement {
    pub fn new(widget: &CustomPaint) -> Self {
        ...
        //     log::warn!("Got signal to render CustomPaint");
        // });
...
impl AsRef<RefCell<WidgetComponent>> for CustomPaintElement {
...
impl Element for CustomPaintElement {
    ...
    fn update(&self, dt: f32) {
        log::info!("Update CustomPaintElement");
        let component = self.component.borrow();

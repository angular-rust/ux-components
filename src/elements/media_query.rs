...
struct MediaQueryState {
    depth_idx: f32, // = 0.0;

    mouse_down: bool, // = false;
}

/// A MediaQuery is a layout-only object and it do not requires a rendering instance.
/// It and handles all incoming events, propagating them to the children.
/// Additional Signals: none

pub struct MediaQueryElement {
    ...
    state: RefCell<MediaQueryState>,

    // The node in layout system
    pub node: Node,
}

impl Debug for MediaQueryElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("MediaQueryElement").finish()
    }
}

impl MediaQueryElement {
    pub fn new(widget: &MediaQuery) -> Self {
        ...
        //     log::warn!("Got signal to render MediaQuery");
        // });
...
impl AsRef<RefCell<WidgetComponent>> for MediaQueryElement {
...
impl Element for MediaQueryElement {
    ...
    fn update(&self, dt: f32) {
        log::info!("Update MediaQueryElement");
        let component = self.component.borrow();

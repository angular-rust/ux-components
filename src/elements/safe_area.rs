...
struct SafeAreaState {
    depth_idx: f32, // = 0.0;

    mouse_down: bool, // = false;
}

/// A SafeArea is a layout-only object and it do not requires a rendering instance.
...
pub struct SafeAreaElement {
    ...
    state: RefCell<SafeAreaState>,

    // The node in layout system
    pub node: Node,
}

impl Debug for SafeAreaElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("SafeAreaElement").finish()
    }
}

impl SafeAreaElement {
    pub fn new(widget: &SafeArea) -> Self {
        ...
        //     log::warn!("Got signal to render SafeArea");
        // });
...
impl AsRef<RefCell<WidgetComponent>> for SafeAreaElement {
...
impl Element for SafeAreaElement {
    ...
    fn update(&self, dt: f32) {
        log::info!("Update SafeAreaElement");
        let component = self.component.borrow();

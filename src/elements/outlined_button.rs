...
pub struct OutlinedButtonElement {
...
impl Debug for OutlinedButtonElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("ButtonElement").finish()
    }
}

impl OutlinedButtonElement {
    pub fn new(widget: &OutlinedButton) -> Self {
...
impl AsRef<RefCell<WidgetComponent>> for OutlinedButtonElement {
    fn as_ref(&self) -> &RefCell<WidgetComponent> {
        self.component.as_ref()
    }
}

impl Element for OutlinedButtonElement {
    fn mousedown(&self, e: &mut MouseEvent) {
        ...
        log::info!("OutlinedButtonElement clicked");
        // self.state.borrow_mut().mouse_down = true;
        // let component = self.component.borrow();
        // component.onmousedown.emit(e);

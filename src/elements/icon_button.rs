...
impl Debug for IconButtonElement {
...
impl IconButtonElement {
    pub fn new(widget: &IconButton) -> Self {
...
impl AsRef<RefCell<WidgetComponent>> for IconButtonElement {
    fn as_ref(&self) -> &RefCell<WidgetComponent> {
        self.component.as_ref()
    }
}

impl Element for IconButtonElement {
    fn mousedown(&self, e: &mut MouseEvent) {
        ...
        log::info!("IconButtonElement clicked");
        // self.state.borrow_mut().mouse_down = true;
        // let component = self.component.borrow();
        // component.onmousedown.emit(e);
    ...
    fn render(&self) {
        ...
        log::warn!("Render IconButtonElement");
    ...
    fn relayout(&self, origin: Point2<f32>) {
        let update_childs = match LayoutSystem::layout(self.node) {
            Ok(layout) => {
                ...
                log::warn!(
                    "Relayout IconButtonElement {}x{} {}x{}",
                    comp.x,
                    comp.y,
                    comp.w,
                    comp.h

...
pub struct ElevatedButtonElement {
...
impl Debug for ElevatedButtonElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("ButtonElement").finish()
    }
}

impl ElevatedButtonElement {
    pub fn new(widget: &ElevatedButton) -> Self {
...
impl AsRef<RefCell<WidgetComponent>> for ElevatedButtonElement {
    fn as_ref(&self) -> &RefCell<WidgetComponent> {
        self.component.as_ref()
    }
}

impl Element for ElevatedButtonElement {
    fn mousedown(&self, e: &mut MouseEvent) {
        ...
        log::info!("ElevatedButtonElement clicked");
        // self.state.borrow_mut().mouse_down = true;
        // let component = self.component.borrow();
        // component.onmousedown.emit(e);
    ...
    fn render(&self) {
        ...
        log::warn!("Render ElevatedButtonElement");
    ...
    fn relayout(&self, origin: Point2<f32>) {
        let update_childs = match LayoutSystem::layout(self.node) {
            Ok(layout) => {
                ...
                log::warn!(
                    "Relayout ElevatedButtonElement {}x{} {}x{}",
                    comp.x,
                    comp.y,
                    comp.w,
                    comp.h

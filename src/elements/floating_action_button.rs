...
pub struct FloatingActionButtonElement {
...
impl Debug for FloatingActionButtonElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("ButtonElement").finish()
    }
}

impl FloatingActionButtonElement {
    pub fn new(widget: &FloatingActionButton) -> Self {
...
impl AsRef<RefCell<WidgetComponent>> for FloatingActionButtonElement {
    fn as_ref(&self) -> &RefCell<WidgetComponent> {
        self.component.as_ref()
    }
}

impl Element for FloatingActionButtonElement {
    fn mousedown(&self, e: &mut MouseEvent) {
        ...
        log::info!("FloatingActionButtonElement clicked");
        // self.state.borrow_mut().mouse_down = true;
        // let component = self.component.borrow();
        // component.onmousedown.emit(e);
    ...
    fn render(&self) {
        ...
        log::warn!("Render FloatingActionButtonElement");
    ...
    fn relayout(&self, origin: Point2<f32>) {
        let update_childs = match LayoutSystem::layout(self.node) {
            Ok(layout) => {
                ...
                log::warn!(
                    "Relayout FloatingActionButtonElement {}x{} {}x{}",
                    comp.x,
                    comp.y,
                    comp.w,
                    comp.h

...
struct SingleChildScrollViewState {
    depth_idx: f32, // = 0.0;

    mouse_down: bool, // = false;
}

/// A SingleChildScrollView is a layout-only object and it do not requires a rendering instance.
/// It and handles all incoming events, propagating them to the children.
/// Additional Signals: none

pub struct SingleChildScrollViewElement {
    ...
    state: RefCell<SingleChildScrollViewState>,

    // The node in layout system
    pub node: Node,
}

impl Debug for SingleChildScrollViewElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("SingleChildScrollViewElement").finish()
    }
}

impl SingleChildScrollViewElement {
    pub fn new(widget: &SingleChildScrollView) -> Self {
        ...
        //     log::warn!("Got signal to render SingleChildScrollView");
        // });
...
impl AsRef<RefCell<WidgetComponent>> for SingleChildScrollViewElement {
...
impl Element for SingleChildScrollViewElement {
    ...
    fn update(&self, dt: f32) {
        log::info!("Update SingleChildScrollViewElement");
        let component = self.component.borrow();

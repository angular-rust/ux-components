...
struct SizedBoxState {
    depth_idx: f32, // = 0.0;

    mouse_down: bool, // = false;
}

/// A SizedBox is a layout-only object and it do not requires a rendering instance.
/// It and handles all incoming events, propagating them to the children.
/// Additional Signals: none

pub struct SizedBoxElement {
    ...
    state: RefCell<SizedBoxState>,

    // The node in layout system
    pub node: Node,
}

impl Debug for SizedBoxElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("SizedBoxElement").finish()
    }
}

impl SizedBoxElement {
    pub fn new(widget: &SizedBox) -> Self {
        ...
        //     log::warn!("Got signal to render SizedBox");
        // });
...
impl AsRef<RefCell<WidgetComponent>> for SizedBoxElement {
...
impl Element for SizedBoxElement {
    ...
    fn update(&self, dt: f32) {
        log::info!("Update SizedBoxElement");
        let component = self.component.borrow();

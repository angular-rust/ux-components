...
struct PreferredSizeState {
    depth_idx: f32, // = 0.0;

    mouse_down: bool, // = false;
}

/// A PreferredSize is a layout-only object and it do not requires a rendering instance.
/// It and handles all incoming events, propagating them to the children.
/// Additional Signals: none

pub struct PreferredSizeElement {
    ...
    state: RefCell<PreferredSizeState>,

    // The node in layout system
    pub node: Node,
}

impl Debug for PreferredSizeElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("PreferredSizeElement").finish()
    }
}

impl PreferredSizeElement {
    pub fn new(widget: &PreferredSize) -> Self {
        ...
        //     log::warn!("Got signal to render PreferredSize");
        // });
...
impl AsRef<RefCell<WidgetComponent>> for PreferredSizeElement {
...
impl Element for PreferredSizeElement {
    ...
    fn update(&self, dt: f32) {
        log::info!("Update PreferredSizeElement");
        let component = self.component.borrow();
    ...
    fn render(&self) {
        ...
        // PreferredSize do not have a render, so we render the child
        self.child.render();
    }

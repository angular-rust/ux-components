...
/// A Positioned is a layout-only object and it do not requires a rendering instance.
...
impl Debug for PositionedElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("PositionedElement").finish()
    }
}

impl PositionedElement {
    pub fn new(widget: &Positioned) -> Self {
        ...
        //     log::warn!("Got signal to render Positioned");
        // });
...
impl AsRef<RefCell<WidgetComponent>> for PositionedElement {
...
impl Element for PositionedElement {
    ...
    fn update(&self, dt: f32) {
        log::info!("Update PositionedElement");
        let component = self.component.borrow();

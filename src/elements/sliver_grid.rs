...
struct SliverGridState {
    depth_idx: f32, // = 0.0;

    mouse_down: bool, // = false;
}

/// A SliverGrid is a layout-only object and it do not requires a rendering instance.
/// It and handles all incoming events, propagating them to the children.
/// Additional Signals: none

pub struct SliverGridElement {
    ...
    state: RefCell<SliverGridState>,

    // The node in layout system
    pub node: Node,
}

impl Debug for SliverGridElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("SliverGridElement").finish()
    }
}

impl SliverGridElement {
    pub fn new(widget: &SliverGrid) -> Self {
        ...
        //     log::warn!("Got signal to render SliverGrid");
        // });
...
impl AsRef<RefCell<WidgetComponent>> for SliverGridElement {
...
impl Element for SliverGridElement {
    ...
    fn update(&self, dt: f32) {
        log::info!("Update SliverGridElement");
        let component = self.component.borrow();

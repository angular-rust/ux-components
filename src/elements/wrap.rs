...
/// A Wrap is a layout-only object and it do not requires a rendering instance.
/// It and handles all incoming events, propagating them to the children.
/// Additional Signals: none

pub struct WrapElement {
    ...
    state: RefCell<WrapState>,

    // The node in layout system
    pub node: Node,
}

impl Debug for WrapElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("WrapElement").finish()
    }
}

impl WrapElement {
    pub fn new(widget: &Wrap) -> Self {
        ...
        //     log::warn!("Got signal to render Wrap");
        // });
    ...
    /// The current focused control, None if none
    pub fn focused(&self) -> Option<&dyn Element> {
        // self.focused
        todo!()
    }
    ...
    /// The current modal control, None if none
    pub fn captured(&self) -> Option<&dyn Element> {
        // self.captured
        todo!()
    }
    ...
    /// The current marked control, None if none
    pub fn marked(&self) -> Option<&dyn Element> {
        // self.marked
        todo!()
    }
...
impl AsRef<RefCell<WidgetComponent>> for WrapElement {

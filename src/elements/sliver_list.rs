...
struct SliverListState {
    depth_idx: f32, // = 0.0;

    mouse_down: bool, // = false;
}

/// A SliverList is a layout-only object and it do not requires a rendering instance.
/// It and handles all incoming events, propagating them to the children.
/// Additional Signals: none

pub struct SliverListElement {
    ...
    state: RefCell<SliverListState>,

    // The node in layout system
    pub node: Node,
}

impl Debug for SliverListElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("SliverListElement").finish()
    }
}

impl SliverListElement {
    pub fn new(widget: &SliverList) -> Self {
        ...
        //     log::warn!("Got signal to render SliverList");
        // });
...
impl AsRef<RefCell<WidgetComponent>> for SliverListElement {
...
impl Element for SliverListElement {
    ...
    fn update(&self, dt: f32) {
        log::info!("Update SliverListElement");
        let component = self.component.borrow();

...
/// A Column is a layout-only object and it do not requires a rendering instance.
...
pub struct ColumnElement {
    ...
    state: RefCell<ColumnState>,
    // The node in layout system
    pub node: Node,
}

impl Debug for ColumnElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("ColumnElement").finish()
    }
}
...
impl ColumnElement {
    pub fn new(widget: &Column) -> Self {
        ...
        //     log::warn!("Got signal to render Column");
        // });
...
impl AsRef<RefCell<WidgetComponent>> for ColumnElement {

...
/// A Opacity is a layout-only object and it do not requires a rendering instance.
...
pub struct OpacityElement {
    ...
    state: RefCell<OpacityState>,

    // The node in layout system
    pub node: Node,
}

impl Debug for OpacityElement {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct("OpacityElement").finish()
    }
}

impl OpacityElement {
    pub fn new(widget: &Opacity) -> Self {
        ...
        //     log::warn!("Got signal to render Opacity");
        // });
...
impl AsRef<RefCell<WidgetComponent>> for OpacityElement {
...
impl Element for OpacityElement {
    ...
    fn update(&self, dt: f32) {
        log::info!("Update OpacityElement");
        let component = self.component.borrow();
